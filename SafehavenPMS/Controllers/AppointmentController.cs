...
namespace SafehavenPMS.Controllers
{
    ...
    public class AppointmentController : Controller
    {
        ...
        public AppointmentController(SafehavenPMSContext context) { _context = context; }

        [HttpGet]
        public async Task<IActionResult> Index()
        {
            // load appointments from DB
            var items = await _context.NewAppointments.ToListAsync();

            // prepare the view model expected by the view
            var vm = new AppointmentPageViewModel
            {
                // // ensure the viewmodel contains the list the view expects
                // WaitlistedPatients = items.Select(a => new AddNewPatientViewModel
                // {
                //     // Map fields from NewAppointment to AddNewPatientViewModel as needed, for example:
            ...
            // optional: supply doctors list for the view (adjust type/projection as needed)
            ViewBag.Doctors = await _context.ClinicalStaffs
                .Select(s => new { s.ClinicalStaffID, Name = s.Firstname + " " + s.Lastname })
                .ToListAsync();
